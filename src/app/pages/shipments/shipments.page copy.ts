...
export class ShipmentsPage implements OnInit {
  ...
  ionViewWillEnter() {
    ...
    this.auth.userData$.subscribe((res: any) => {
      if (res.apiKey) {
        ...
        this.shipmentService.getPODSettings(res.apiKey).subscribe({
          next: (response: any) => {
         ...
         this.shipmentService.getDriverRoute(res.apiKey).subscribe({
           next: (response: any) => {
         ...
         this.shipmentService.getDriverPickupRoute(res.apiKey).subscribe({
           next: (response: any) => {
             this.shipments_route = response;
  ...
  setShipments() {
    this.shipmentService.userShipments$.subscribe({
  ...
  loadData(ev: any) {
    this.page++;
    setTimeout(() => {
      this.shipmentService.page(this.authUser.apiKey, this.page).subscribe({
        next: (response: any) => {
          if (!response) {
            ev.target.complete();
  ...
  allShipments() {
    ...
    this.shipmentService.get(this.authUser.apiKey).subscribe({
      next: (response: any) => {
        this.page = 1;
